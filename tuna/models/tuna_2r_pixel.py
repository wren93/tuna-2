...
class Tuna2RPixel(SiglipMixin, TunaInnerBase):
    """SigLIP-only pixel diffusion Tuna variant (variant B, 7B).

    Architecture:
      - SigLIP2 vision encoder runs on raw pixel values for both conditioning
        and diffusion target (no VAE).
      - Diffusion head learns to predict ``x0`` (clean pixels) directly via
        JiT-style noise scheduling.
      - Optional learnable ``mask_token`` (DeTok integration) for masked-image
        modelling.

    The original tuna code wrapped SigLIP and a (disabled) WAN VAE behind a
    ``SiglipModelWithVAE`` wrapper. Tuna uses the SigLIP2 vision model
    directly; VAE loading is omitted entirely (no ``vae_model_id`` arg).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ada_norm_cfg_feat_thre = 0.8  # Threshold for adaptive CFG normalization
        ...
        #   * image_size override so position_embedding has the trained shape
        #     (e.g. siglip_image_size=512 → position_embedding [1024, 1152]).
        #   * post_layernorm = Identity (tuna replaces it).
        #   * head dropped (tuna uses vision_use_head=False).
        #   * encoder truncated to the requested feature layer
        #     (siglip_feature_layer=-2 → keep first num-1 layers).
        # Pretrained SigLIP weights are warm-loaded for shape-matching keys;
        # the user's checkpoint will overwrite everything during load.
        ...
        # Masked Image (DeTok integration) — Learnable mask token for masked patches.
        # Kept as a controllable feature (training-time random masking only); inpainting
        # paths are dropped per Tuna spec.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if text_labels is not None and image_labels is not None:
            ...
            # Extract target image timesteps (odd indices) for proper time weighting.
    ...
    def _prepare_embeds(
        ...
        if image_latents is not None:
            ...
            # Pixel-based variant: SigLIP2 takes the (noisy) pixel-space input
            # directly (no separate VAE latent space).
            ...
            if self.enable_mask_token and self.training:
                ...
                # Reshape to [B, N_patches, D] for masking
                B, N, D = image_embeds.shape

                # Generate random mask for each sample in batch
                for batch_idx in range(B):
                    # Random masking ratio per sample using min and max range
                    mask_ratio = random.uniform(
                        self.masked_image_ratio_min, self.masked_image_ratio
                    )
                    num_masked = int(N * mask_ratio)

                    if num_masked > 0:
                        # Random sample indices to mask
    ...
    def adaptive_normalize_cfg_feat(self, model_pred_cond, model_pred_cfg):
        """Adaptively normalizes CFG features based on L2 norm ratios.

        Reference: https://arxiv.org/abs/2412.07730
        """
...
class Tuna2RPixelModel(JiTWrapperMixin, TunaWrapperBase):
    ...
    Hydra-instantiable. Trains pixel-space x0-prediction with SigLIP2 features
    serving as both conditioning and the diffusion target. Uses the JiT-style
    noise scheduler from :mod:`tuna.models.misc`.
    """
    ...
    def __init__(
        ...
        # Masked Image (DeTok) configuration — kept for masked-image training.
        # NOTE: the inpainting code path that originally also used the mask
        # token has been dropped per Tuna spec.
        enable_mask_token: bool = False,
    ...
    def build_models(self):
        ...
        self.tuna_model = Tuna2RPixel(**model_config)
        ...
        # Initialize transport for flow matching (kept for backwards compat;
        # JiT noise scheduler below is the primary scheduler in this variant)
    ...
    def prepare_clean_image_embeds(
        self,
        pixel_values_low: torch.Tensor,
        siglip_spatial_shapes: Optional[torch.LongTensor] = None,
        siglip_pixel_attention_mask: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Encode degraded/low-res images through SigLIP2 to get clean image
        embeddings used for reconstruction conditioning."""
        if len(pixel_values_low.shape) == 4:
            pixel_values_low = pixel_values_low.unsqueeze(2)

        dtype = next(self.tuna_model.parameters()).dtype
        clean_image_embeds_siglip = self.tuna_model.encode_pixels_with_siglip(
            pixel_values_low.to(dtype),
            spatial_shapes=siglip_spatial_shapes,
            pixel_attention_mask=siglip_pixel_attention_mask,
        )
        clean_image_embeds = self.tuna_model.siglip_proj(clean_image_embeds_siglip)
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        if pixel_values_low is not None:
            clean_image_embeds = self.prepare_clean_image_embeds(
                pixel_values_low,
                siglip_spatial_shapes=siglip_spatial_shapes,
                siglip_pixel_attention_mask=siglip_pixel_attention_mask,
            )
